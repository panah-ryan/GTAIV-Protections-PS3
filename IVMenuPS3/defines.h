...
struct datBitBuffer
{
	union
	{
		uint8_t* m_ReadBits; //0x00
		uint8_t* m_WriteBits;
	};
	int m_BaseBitOffset; //0x04
	int m_MaxBits; //0x08
	int m_CursorPos; //0x0C
	int m_NumBitsWritten; //0x10
	int m_NumBitsRead; //0x14
	struct
	{
		bool m_IsReadOnly : 1;
	...
	bool CanReadBits(int bits);

	bool CanWriteBits(int bits);

	void BumpReadCursor(int bits)
	{
		m_CursorPos += bits;
	...
	template<typename T>
	bool ReadUns(T* u, int bits)
	{
		if (CanReadBits(bits))
		{
			ReadUnsigned(u, bits, m_CursorPos + m_BaseBitOffset);
			BumpReadCursor(bits);
			return true;
		}
	...
	template<typename T>
	bool WriteUns(T u, int bits)
	{
		if (CanWriteBits(bits))
		{
			WriteUnsigned(static_cast<uint32_t>(u), bits, m_CursorPos + m_BaseBitOffset);
			BumpWriteCursor(bits);
			return true;
		}
...
public:
	char _0x04[0x60];
	CNetworkObject* m_NetObject; //0x64
...
}; static_assert(sizeof(CEntity) == 0x68, "Incorrect sizeof CEntity");
...
}; static_assert(sizeof(CDynamicEntity) == 0x108, "Incorrect sizeof CDynamicEntity");
...
}; static_assert(sizeof(CPhysical) == 0x208, "Incorrect sizeof CPhysical");
...
public:
	char _0x00[0x74];
	char m_Gamertag[16]; //0x74
	char _0x84[0x520];
	CPed* m_PlayerPed; //0x5A4
